...
2. Нажать `Говорить с Альтроном`.
3. Говорить обычным голосом после появления состояния `Слушаю`.
4. Дождаться состояний `Думаю` и `Говорю`. Распознанная фраза и ответ остаются видимыми,
   ответ также озвучивается локальным TTS.
5. Для проверяемого действия открыть `Operator`, продиктовать команду и отдельно проверить
